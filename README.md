...
알렉스 넷과 VGG19 , RESNET을 토대로 깊이가 깊어짐이 능사는 아니라는 실험 결과를 통해
고유 네트워크의 깊이는 적당하게 만들기로 결정하였다.

그리고 몇몇 실험을 통해 활성화함수의 차이로 성능에서 많은 차이를 보인다는 것을 알게 되었다.
에포크와 로스 , 러닝레이트와 배치사이즈 등의 실험은 핵심적인 성능차이가 없을 것이라 예상되어 실험을 진행하진 않았다.
( 단순한 Task를 해결하는 문제라고 생각했기 때문에 )

결과적으로 낮은 횟수의 에포크를 통해
과거의 SOTA 모델들과 유사한 성능을 보이는 네트워크를 학습 시켰다는 점에서
큰 의의를 둔다.
